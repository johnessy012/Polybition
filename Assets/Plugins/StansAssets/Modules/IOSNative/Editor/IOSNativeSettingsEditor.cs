	...
	public Texture[] ToolbarImages {
		get {
			if(_ToolbarImages == null) {
				Texture2D appstore =  Resources.Load("appstore") as Texture2D;
				Texture2D gamecenter =  Resources.Load("gamecenter") as Texture2D;
				Texture2D media =  Resources.Load("media") as Texture2D;
//				Texture2D sharing =  Resources.Load("sharing") as Texture2D;
				Texture2D settings =  Resources.Load("settings") as Texture2D;				
				Texture2D ios =  Resources.Load("ios") as Texture2D;
				Texture2D camera =  Resources.Load("camera") as Texture2D;
				Texture2D editorTesting = Resources.Load("editorTesting") as Texture2D;
				Texture2D xcode = Resources.Load("xcode") as Texture2D;
	...
	public override void OnInspectorGUI()  {
		...
		IOSNativeSettings.Instance.ToolbarIndex = GUILayout.Toolbar(IOSNativeSettings.Instance.ToolbarIndex, ToolbarImages, new GUILayoutOption[]{ GUILayout.Height(35f)});
	...
	private void MoreActions() {
		...
		if (IOSNativeSettings.Instance.ShowOtherParams) {
			...
			if(GUILayout.Button("Load Example Settings",  GUILayout.Width(140))) {
				...
				CFBundleURLTypes.ArrayType = SA.IOSDeploy.PlistValueTypes.String;

				SA.IOSDeploy.Variable CFBundleURLName =  new SA.IOSDeploy.Variable();
				CFBundleURLName.StringValue = "CFBundleURLName";
				...
				CFBundleURLName.StringValue = "CFBundleURLSchemes";
				CFBundleURLName.Type = SA.IOSDeploy.PlistValueTypes.String;

				SA.IOSDeploy.Variable Scheme =  new SA.IOSDeploy.Variable();
				Scheme.StringValue = "isn";
				Scheme.Type = SA.IOSDeploy.PlistValueTypes.String;

			
				CFBundleURLTypes.PlistVariables.Add(CFBundleURLName);
				CFBundleURLTypes.PlistVariables.Add(CFBundleURLSchemes);


				CFBundleURLSchemes.PlistVariables.Add (Scheme);
